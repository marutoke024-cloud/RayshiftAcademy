...
- [x] **Phase 3: 復習・モチベーション機能**
  - [x] 復習モード（全ステップ完了で解放・解説全文／初回＆最新メモ／フィードバック／成長記録）
  - [x] マスタリーバッジ画像対応（`assets/badges/mastery_[0-5].png`、未登録時は SVG 代替）
  - [x] 称号システム（個別称号＋複数完了で上位称号）
  - [x] 今日のおすすめ（ステップ単位：学習中＞完了で mastery 低い順）
  - [x] ミックス復習（完了ステップを横断ランダム出題）
- [x] **Phase 4: モバイル対応の最適化**
  - [x] デバイス判定の強化（画面幅 + userAgent + iPadOS 検出）
  - [x] スマホ／タブレットは復習専用（学習フローを遮断し復習へ誘導）
  - [x] モバイル用トップ最適化（カリキュラム一覧＋称号、カードタップで復習へ直行）
  - [x] Galaxy S25 / iPad Air 等向けのレスポンシブ調整（タップ領域・狭幅・セーフエリア）

> テーマカラーは Microsoft Teams 風（Fluent ライト配色・Teams パープル `#5B5FC7`）。

### デバイス対応について
- **PC（Windows 等・画面幅 1024px 以上）**: 全機能（学習・復習・インポート）
- **スマホ／タブレット（Galaxy S25, iPad Air など）**: 復習モードのみ
  - iPad Air 10.5 は横向きだと幅 1112px になりますが、userAgent 判定により**向きに関わらず復習専用**になります。
  - iPadOS 13+ は userAgent が Mac に偽装されるため、タッチ点数（`maxTouchPoints`）でも iPad を検出します。
